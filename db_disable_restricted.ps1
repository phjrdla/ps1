<#	
.SYNOPSIS
db_disable_restricted invokes pl/sql procedure solife_util.disable_restricted on specified database
	
.DESCRIPTION
db_disable_restricted invokes pl/sql procedure solife_util.disable_restricted
to kill user sessions and disable restricted mode on specified database

.Parameters oracleSid
oracleSid is used to setup Oracle environment variable ORACLE_SID. 
revokeFrom specifies from which account to revoke 'restricted session' 
mode is used to choose the type of run, 'FALSE' for a simulation, 'TRUE' for a live run

.Example db_disable_restricted -oracleSid orasolifedev -mode 'FALSE'		
#>

[CmdletBinding()] param(
  [Parameter(Mandatory=$True) ] [string]$oracleSid,
  [Parameter(Mandatory=$True) ] [string]$revokeFrom,
...
$sql = @"
set serveroutput on
execute solife_util.disable_restricted('$revokeFrom', $mode);
